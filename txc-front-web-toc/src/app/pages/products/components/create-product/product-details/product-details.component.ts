...
import { debounceTime, distinctUntilChanged, map, merge, Observable, OperatorFunction, Subject } from 'rxjs';
...
const STATES: { name: string, flag: string }[] = [
...
export class ProductDetailsComponent implements OnInit {
  ...
  voucherRules = [
    {
      selected: true,
      ruleName: 'Test 3rd party rule name 001',
      createdBy: 'Generated By 3rd party',
      programName: 'ProgramName',
      requireExpDate: 'N',
      pinCodeType: 'Not required',
      hasMultipleBarcode: 'N',
      showCodeType: 'Code 39',
      displayText: 'N',
      onDemand: 'N',
      onDemandBy: '',
      vendorCode: '--'
    ...
    {
      selected: false,
      ruleName: 'Test 3rd party rule name 002',
      createdBy: 'Generated By 3rd party',
      programName: 'ProgramName',
      requireExpDate: 'N',
      pinCodeType: 'Not required',
  ...
  formatter = (result: { name: string, flag: string }) => result?.name;
  ...
  searchBrand: OperatorFunction<string, readonly { name: string, flag: string }[]> = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$).pipe(
      map(term => (term === '' ? this.statesList
        : this.statesList.filter(v => v.name.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 10))
    );
  }

  searchMerchant: OperatorFunction<string, readonly { name: string, flag: string }[]> = (text$: Observable<string>) => {
    const debouncedText$ = text$.pipe(debounceTime(200), distinctUntilChanged());
    // const clicksWithClosedPopup$ = this.click$.pipe(filter(() => !this.instance.isPopupOpen()));
  ...
  addMerchant() {
    this.availableMerchants.push(this.f.availableMerchants.value);
    this.f.availableMerchants.reset();
  }
