...
import { Shops } from 'src/assets/data/mock-data';
...
export class ShopService {
  ...
  private _getMerchantShop(): Observable<any> {
    ...
    const url = `http://${environment.apiUrl}api/MerchantShop?${this.searchTerm != '' ? 'SearchKeyword=' : ''}${searchTerm}${this.merchantId != 0 ? '&MerchantId=' : ''}${this.merchantId != 0 ? merchantId : ''}&RowCount=${pageSize}&PageNumber=${page}`;
    return this.http.get(url, { headers: this.getHeaders() }) as Observable<GetShopResponse>;
  }
  ...
  getShop(shopId: number): Observable<BaseResponse> {
    const url = `http://${environment.apiUrl}api/MerchantShop/GetById?ShopId=${shopId}`;
    return this.http.get(url, { headers: this.getHeaders()}) as Observable<BaseResponse>;
  }

  createShop(body: any): Observable<BaseResponse> {
    const url = `http://${environment.apiUrl}api/MerchantShop`;
