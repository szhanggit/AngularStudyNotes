...
import { AmmTokenService } from 'src/app/core/service/security/amm-token.service';
import { CookieManagerService } from 'src/app/core/service/security/coockie-manager/cookie-manager.service';
...
export class PrivateLayoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.selectedTenant = config.tenantCountry;
    ...
    this.eventService.subscribe(LayoutEventType.CHANGE_TENANT_COUNTRY, (type) => {
      this.selectedTenant = type as SideBarWidth;
    });
  ...
  // Redirect to main move url 
  redirectToMain() {
    if (window.location.href.includes("maintenance")) {
      return;
    }
    window.location.href = environment.landingUrl + '/move'
  }
}
