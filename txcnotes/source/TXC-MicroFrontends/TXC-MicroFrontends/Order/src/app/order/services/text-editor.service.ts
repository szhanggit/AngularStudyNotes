...
export class TextEditorService {
  ...
  convertHtmlToPlainText(html: string) {
    ...
    plainText = plainText.replace(this.PATTERN.LI, (_: any, li: string) => {
      return `${this.PATTERN.BULLET}${li}${this.PATTERN.LINEBREAK}`;
    });
    ...
    plainText = plainText
      .trim()
      .replace(this.PATTERN.EXTRA_SPACE, String.fromCharCode(160));

    // replace right arrow
    plainText = plainText.replace(
      this.PATTERN.RIGHT_ARROW_HTML,
      this.PATTERN.RIGHT_ARROW
    );
