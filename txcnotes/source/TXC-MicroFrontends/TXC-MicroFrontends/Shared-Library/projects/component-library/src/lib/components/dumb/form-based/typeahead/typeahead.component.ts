...
export class TypeaheadLibComponent extends BaseControlComponent implements OnInit {
  ...
  onTypeAheadChanged() {
    const selectItem = this.typeaheadModel.list!.find(
      (list) => list.keyword.toLowerCase() === this.value.toLowerCase()
    );
    if (selectItem) {
      ...
      map((term) =>
        (term === ''
          ...
          : this.typeaheadModel.list!.filter(
              (v) => v.keyword.toLowerCase().indexOf(term.toLowerCase()) > -1
            )
        ).slice(0, 20)
      )
    );
